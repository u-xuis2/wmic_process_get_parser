...
"""
デバッグ機能付きWMIC PROCESS GET情報をパースしてJSON形式で出力するツール
"""
...
def execute_wmic():
    ...
    try:
        ...
        for encoding in encodings:
            try:
                print(f"デバッグ: エンコーディング '{encoding}' を試行中...", file=sys.stderr, flush=True)
                ...
                if result.returncode == 0:
                    ...
                    if result.stdout and 'ProcessId=' in result.stdout:
                        ...
                        print(f"デバッグ: 出力の最初の200文字: {repr(result.stdout[:200])}", file=sys.stderr, flush=True)
                        break
                    else:
...
def parse_wmic_output(output):
    ...
    # 改行コードを正規化（Windows対応）
    print(f"デバッグ: 元の出力長: {len(output)}", file=sys.stderr, flush=True)
    print(f"デバッグ: 最初の100文字: {repr(output[:100])}", file=sys.stderr, flush=True)
    
    output = output.replace('\r\n', '\n').replace('\r', '\n')
    
    print(f"デバッグ: 正規化後の出力長: {len(output)}", file=sys.stderr, flush=True)
    print(f"デバッグ: 正規化後の最初の100文字: {repr(output[:100])}", file=sys.stderr, flush=True)
    
    lines = output.split('\n')
    print(f"デバッグ: 総行数: {len(lines)}", file=sys.stderr, flush=True)
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        # 最初の20行をデバッグ出力
        if line_num <= 20:
            print(f"デバッグ: 行{line_num}: {repr(original_line)}", file=sys.stderr, flush=True)
        ...
        if '=' in line:
            ...
            print(f"デバッグ: キー='{key}', 値='{value}'", file=sys.stderr, flush=True)
            
            if key == 'Caption':
                current_process['caption'] = value
                print(f"デバッグ: captionを設定: {current_process}", file=sys.stderr, flush=True)
                ...
                print(f"デバッグ: commandlineを設定: {current_process}", file=sys.stderr, flush=True)
            elif key == 'Name':
                current_process['name'] = value
                print(f"デバッグ: nameを設定: {current_process}", file=sys.stderr, flush=True)
            elif key == 'ProcessId':
                # ProcessIdでプロセス情報を完成させる
                if value:
                    try:
                        current_process['process_id'] = int(value)
                        print(f"デバッグ: process_idを設定: {current_process}", file=sys.stderr, flush=True)
                    except ValueError:
                        current_process['process_id'] = None
                        print(f"デバッグ: process_idをNoneに設定: {current_process}", file=sys.stderr, flush=True)
                
                # プロセス情報を追加
                if current_process and 'process_id' in current_process:
                    print(f"デバッグ: プロセス追加: {current_process}", file=sys.stderr, flush=True)
                    processes.append(current_process.copy())
                
                # 次のプロセス用に初期化
                print(f"デバッグ: 次のプロセス用に初期化", file=sys.stderr, flush=True)
                current_process = {}
    
    # ProcessIdで処理済みのため、最後の処理は不要
    
    print(f"デバッグ: 総プロセス数: {len(processes)}", file=sys.stderr, flush=True)
    return processes
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WMIC PROCESS GET情報をパースしてJSON形式で出力するツール（デバッグ版）"
    )
    parser.add_argument(
        "output_file",
        nargs="?",
